...
function generateConclusion(W, H_total, Power_el) {
    ...
    if (W > 3.0) {
        conBlock.className = "conclusion-box conclusion-danger";
        conBlock.innerHTML = `<b>Аварийный режим работы!</b> Выбранный диаметр трубы не подходит. Скорость потока хладагента составляет <b>${W.toFixed(2)} м/с</b> (критический порог 3 м/с). Это неизбежно приведет к разрушению арматуры и кавитации. Срочно увеличьте диаметр.`;
    } else if (W > 1.5) {
        conBlock.className = "conclusion-box conclusion-warning";
        conBlock.innerHTML = `<b>Параметры вне оптимума (Завышенная скорость):</b> Труба выбрана, но скорость потока <b>${W.toFixed(2)} м/с</b> выше рекомендуемой нормы (1.5 м/с). Работа будет сопровождаться шумом и огромными потерями напора насоса (требуется <b>${H_total.toFixed(1)} м вод. ст.</b>). Рекомендуется увеличить диаметр контура.`;
    } else if (W < 0.6) {
        conBlock.className = "conclusion-box conclusion-warning";
        conBlock.innerHTML = `<b>Параметры вне оптимума (Заниженная скорость):</b> Скорость потока <b>${W.toFixed(2)} м/с</b> ниже минимального порога самоочищения (0.6 м/с). Существует высокий риск заиливания контура охлаждения. Рекомендуется заузить диаметр.`;
    } else {
        conBlock.className = "conclusion-box conclusion-success";
        conBlock.innerHTML = `<b>Расчёт успешно завершён. Система в идеальном допуске!</b> Скорость хладагента составляет <b>${W.toFixed(2)} м/с</b> (норма 0.6 — 1.5 м/с). Насос работает экономично, требуемая электрическая мощность: <b>${Power_el.toFixed(2)} кВт</b>.`;
    }
}
